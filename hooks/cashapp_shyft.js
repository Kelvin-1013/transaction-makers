...
export const useCashApp = () => {
    ...
    const TrackStart = ({addressTemp,isTrack}) => {
        ...
        // alert(trackAddress);
        getTransactionHistory(addressTemp);
    };
    const getTransactionHistory = async (address) => {
        alert(API_KEY);
        try {
            const response = await axios.get('https://api.shyft.to/sol/v1/transaction/history',{
                // const response = await axios.get('https://api.shyft.to/sol/v1/wallet/transaction_history',{
                params: {
                    network: 'devnet',
                    // account: {trackAddress},
                    account: {address},
                    // wallet: {address},
                    tx_num: 1 // Number of transactions to retrieve
                },
                headers: {
                    'x-api-key': API_KEY
                }
